"""Cryptopals Challenges: Test Challenge 38: Offline dictionary attack on simplified SRP: Server."""

import sys
import utils.cp_aux_utils as utils

title = "Challenge 38: Offline dictionary attack on simplified SRP: Server"
...
def request_handler(rh):
    ...
    try:
        ...
        debug_msg("-" * 60)
        u = int(utils.rand_bytes(16).hex(), 16)
        ...
        sock_io.writenum(B)
        debug_msg("done:\n  B = [{0}]".format(B))
        debug_msg("server: Sending 'u'...", end = '', flush = True)
        sock_io.writenum(u)
        debug_msg("done:\n  u = [{0}]".format(u))
